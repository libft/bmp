...
static void	fill_header(t_ft_bmp *self, char *result)
{
...
static void	fill_body(t_ft_bmp *self, char *result)
{
	const size_t	row_padding = (4 - (self->width * 3) % 4) % 4;
	size_t			offset;
	size_t			y;
...
t_err	ft_bmp_serialize(t_ft_bmp *self, char **out, size_t *out_length)
{
	const size_t	row_padding = (4 - (self->width * 3) % 4) % 4;
	const size_t	row_size = self->width * 3 + row_padding;
	const size_t	whole_size = row_size * self->height;
